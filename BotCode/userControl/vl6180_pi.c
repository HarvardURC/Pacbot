...
int change_address(vl6180 handle, char new_addr){
	...
	ioctl(handle, I2C_SLAVE, 0x029);
	write_byte(handle, 0x0212 , new_addr);
	ioctl(handle, I2C_SLAVE, new_addr);
	printf("address slave is %d\n", read_byte(handle, 0x212));
	//ioctl(handle, I2C_SLAVE, new_addr);
	//printf("i2c slave %d\n", I2C_SLAVE);
	//printf("%d\n", read_byte(handle, 0x0212));
	//printf("revert %d\n", read_byte(handle, 0x0212));
	return 0;
}
